...
pivot_game_data <- function(df, usernames ){
    df %>%
        ...
        mutate(
            ...
            lead_indicator = ifelse(pokemon == 'lead1' , 1, 0)
        )  %>%
...
create_summary_by_pokemon <- function(longdata, reliable_only = FALSE){
    df2 %>%
        ...
        summarize(
            num_matches = n(),
            win_rate = round(1 - mean(win_flag), 3),
            std_err = round(sqrt(win_rate/(1-win_rate)/num_matches), 3),
...
ui <- fluidPage(
    ...
    # App title ----
    titlePanel("Analytic: The Pokemon Showdown! Parser"),
    ...
    sidebarLayout(
        ...
        sidebarPanel(
            img(src='137Porygon_Smile.png', height=100, width=100),
            
            # Input: Select a file ----
            fileInput("file1", "1) Choose Showdown Replay Files",
                      multiple = TRUE,
                      accept = c("text/csv",
                                 "text/,text/plain",
                                 ".html")),
            
            textInput('users', "2) Enter your usernames used (case-sensitive, separate by single space).  This is only to distinguish you and your opponents"),
            
            
            h5("3) Download Parsed Data - Raw and/or Summary"),
            downloadButton("rawdata", "Download Raw Data .csv"),
            downloadButton('rawlong', "Download Raw Data in Long Format .csv"),
            downloadButton("matchupbymon", "Download Matchup Summary by Pokemon .csv"),
            
            h5("I DO NOT KEEP YOUR DATA!")
        ...
        mainPanel(
            ...
            # Output: Data file ----
            DTOutput("contents"),
            DTOutput("pokemonsummary"),
            DTOutput("longdata_view")
...
server <- function(input, output) {
    ...
    rawdata_df <- reactive(
        create_raw_data(input$file1$datapath)
    )
    
    longdata_df <- reactive(
        pivot_game_data(rawdata_df(), usernames = input$users)
    )
    
    pokemonsummary_df <- reactive(
        create_summary_by_pokemon(longdata_df())
    )
    ...
    output$contents <- renderDT({
          ...
          datatable(
            { rawdata_df() },
            caption = "Raw Match Data",
        )
        
    })
    
    output$pokemonsummary <- renderDT({
        datatable(pokemonsummary_df() ,
        
        caption = 'Summary by Pokemon')
    })
    
    output$longdata_view <- renderDT({
        datatable(longdata_df(), caption = 'raw data (long)' )
    })
    
    output$rawdata <- downloadHandler(
        filename = "showdown-rawdata.csv",
        content = function(file){
            readr::write_csv(rawdata_df(), file )
        }
    )
    
    output$rawlong <- downloadHandler(
        filename = 'showdown-rawdata-long.csv',
        content = function(file){
            readr::write_csv(longdata_df(), file)
        }
    )
    
    output$matchupbymon <- downloadHandler(
        filename = 'matchup-by-mon.csv',
        content = function(file){
            readr::write_csv(pokemonsummary_df(), file)
        }
    )
